...
extern "C" void render_kernel(float4 *pos, unsigned int mesh_width, unsigned int mesh_height, float time);
...
void runCuda(struct cudaGraphicsResource **vbo_resource)
{
	...
	render_kernel(dptr, mesh_width, mesh_height, g_fAnim);
...
void runAutoTest(int devID, char **argv, char *ref_file)
{
	...
	// execute the kernel
	render_kernel((float4 *)d_vbo_buffer, mesh_width, mesh_height, g_fAnim);
